...
fn temporary_csv(content: &str) -> NamedTempFile {
    ...
    file.flush()
        .expect("temporary CSV should be flushed");
...
#[cfg(test)]
mod tests {
    use anomali_rs::csv_reader::{read_column, CsvReadError};
    ...
    fn preserves_missing_values() {
        ...
        let result = read_column(file.path(), "amount")
            .expect("amount column should be read");
    ...
    fn handles_quoted_fields() {
        ...
        let result = read_column(file.path(), "supplier")
            .expect("supplier column should be read");

        assert_eq!(
            result.values,
            vec![
                "Company, Inc.",
                "Example \"Quoted\" Company",
            ]
        );
    }
    ...
    fn accepts_a_header_only_csv() {
        ...
        let result = read_column(file.path(), "amount")
            .expect("header-only CSV should be valid");
    ...
    fn returns_error_when_file_does_not_exist() {
        let path = std::path::Path::new(
            "this-file-should-not-exist.csv",
        );
